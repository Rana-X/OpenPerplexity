...
const AssistantSteps = ({
  ...
  useEffect(() => {
    if (researchEnded && isLast) {
      setIsExpanded(false);
    } else if ((status === 'answering' || status === 'error') && isLast) {
      setIsExpanded(true);
    }
      ...
      >
        <div className="flex items-center gap-2">
          {isErrored ? (
            ...
            <CircleCheckBig className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
          ) : (
            <Brain className="w-4 h-4 text-black dark:text-white" />
          )}
          <span className="text-sm font-medium text-black dark:text-white">
            {headerTitle} ({block.data.subSteps.length}{' '}
        ...
        {isExpanded && (
          ...
          >
            <div className="p-3 space-y-2">
              {isErrored && (
                <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-100">
                  Search stopped before the app could produce a final answer.
                  Retry the query now that search is available again.
                </div>
              )}
              {block.data.subSteps.map((step, index) => {
                const isLastStep = index === block.data.subSteps.length - 1;
                const isStreaming =
                  isAnswering && loading && isLastStep && !researchEnded;
