...
export const POST = async (req: Request) => {
  try {
    ...
    const demoSessionId = demoMode
      ? getDemoSessionIdFromHeaders(req.headers)
      : null;

    if (demoMode && !demoSessionId) {
      return Response.json(
        { message: 'Missing demo session id.' },
        { status: 400 },
      );
    }

    if (demoMode) {
      UploadManager.cleanupExpiredDemoSessions();
    }
