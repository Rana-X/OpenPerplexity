...
const createId = (bytes: number) => crypto.randomBytes(bytes).toString('hex');
...
export const ChatProvider = ({
  ...
  const [isMessagesLoaded, setIsMessagesLoaded] = useState(demoMode);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [demoMode, chatId, isMessagesLoaded, newChatCreated, messages.length]);
  ...
  const sendMessage: ChatContext['sendMessage'] = async (
    ...
    const activeChatId = chatId ?? createId(20);

    if (!chatId) {
      setChatId(activeChatId);
    }

    messageId = messageId ?? createId(7);
    const backendId = createId(20);
    ...
    if (!res.ok) {
      ...
      const messageText =
        errorBody?.message || 'An error occurred while sending the message.';
