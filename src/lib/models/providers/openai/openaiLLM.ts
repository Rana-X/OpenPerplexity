...
class OpenAILLM extends BaseLLM<OpenAIConfig> {
  ...
  private shouldFallbackFromStructuredOutputs(err: unknown) {
    return (
  ...
  private async generateObjectWithoutStructuredOutputs<T>(
    ...
    const response = await this.openAIClient.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `Return only valid JSON that matches this JSON schema: ${schema}. Do not include markdown fences or any extra text.`,
        },
    ...
    if (response.choices && response.choices.length > 0) {
      return this.parseObjectResponse(input.schema, response.choices[0].message.content) as T;
    }
  ...
  async generateObject<T>(input: GenerateObjectInput): Promise<T> {
    try {
      ...
      if (response.choices && response.choices.length > 0) {
        return this.parseObjectResponse(
          input.schema,
          response.choices[0].message.content,
        ) as T;
      }
    } catch (err) {
      if (this.shouldFallbackFromStructuredOutputs(err)) {
        console.warn(
          `Structured outputs are unsupported for ${this.config.model}; falling back to plain JSON prompting.`,
        );
        return this.generateObjectWithoutStructuredOutputs<T>(input);
      }
